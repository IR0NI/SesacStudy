...
2. 페이징 기법 : 가상메모리 사용. 외부단편화 해결 방법. 가상메모리를 같은 크기의 블록으로 나눈 것을 페이지라고 하고
메모리를 페이지와 같은 크기로 나눈 것을 프레임이라고 한다.
사용하지 않는 프레임을 페이지에 옮기고 필요한 메모리를 페이지 단위로 프레임에 옮겨주는 기법
3. 세그멘테이션 기법 : 내부단편화 해결
가상메모리를 서로 다른 크기로 분할해서 메모리를 할당하여 실제 메로리 주소로 변환하여 사용하는 기법

댕글링 포인터 해결방법 : 스마트 포인터 사용
레퍼런스 카운트를 이용하여 참조하는 카운트가 없을 경우 메모리를 제거하는 방식

c++에서 제곡하는 스마트포인터는 std namespace 안에 들어가 있다.
c++의 스마트 포인터 종류
std::unique_ptr : 하나의 스마트 포인터만 소유한다.
단독소유. 복사는 안되고 이동은 됨
std::shared_ptr : 여러 객체가 하나의 공간을 공유하는 경우
레퍼런스 카운트를 이용하여 참조하는 횟수를 만들고 레퍼런스 카운트가 0이되면 
메모리를 제거하는 방식이다.
속도가 느리다
